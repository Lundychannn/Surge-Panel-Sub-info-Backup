 ...
 * 更新時間：2025年4月2日
 ***********/
console.log("【1號機場】面板刷新執行於: " + new Date().toISOString());
...
(async () => {
  ...
  if (!info) {
    console.log("【1號機場】未取得流量資訊，可能伺服器未返回 subscription-userinfo。");
    return $done();
  }
...
function getUserInfo(url) {
  ...
  return new Promise((resolve, reject) =>
    $httpClient.get(request, (err, resp) => {
      console.log("【1號機場】發送請求至：", request.url);
      if (err != null) {
        console.log("【1號機場】請求錯誤:", err);
        reject(err);
        return;
      }
      if (resp.status !== 200) {
        console.log("【1號機場】HTTP 狀態碼:", resp.status);
        reject(resp.status);
        return;
      }
      console.log("【1號機場】回傳 Headers:", JSON.stringify(resp.headers, null, 2));
      let header = Object.keys(resp.headers).find((key) => key.toLowerCase() === "subscription-userinfo");
      if (header) {
        resolve(resp.headers[header]);
        return;
      }
      reject("【1號機場】伺服器未包含 subscription-userinfo header");
    })
  );
}
